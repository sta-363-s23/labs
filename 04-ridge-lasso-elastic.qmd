...
6. Fit an elastic net model on the training set with $\lambda$ and $\alpha$ chosen using cross validation. Create a Figure with the estimated test root mean squared error on the y-axis and $\lambda$ on the x-axis with lines colored by the mixture chosen. 
